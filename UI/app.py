...
def direct_page():
    select = request.form.get('recommendation_type')
    input_value = request.form.get('input_value')

    if select == "user_id":
        return show_recommendation(input_value)
    elif select == "content_based":
        return show_content_based_recommendation(input_value)


@app.route("/content_based", methods=["POST"])
def show_content_based_recommendation(movie_name=None):
    if not movie_name:
        movie_name = request.form.get("input_value")
...
def show_recommendation(user_id=None):
    if not user_id:
        user_id = int(request.form.get("input_value"))
    ...
    try:
        ...
        for x in matrix_data:
            print(x)
            imgscrape.download_poster(x, "download")
